...
public class DragAndDropManipulator : PointerManipulator
{
    ...
    private void PointerCaptureOutHandler(PointerCaptureOutEvent evt)
    {
        if (enabled)
        {
            ...
            if(closestOverlappingSlot != null && closestOverlappingSlot != parent)
            {
                if(closestOverlappingSlot.childCount > 0)
                {
                    //TODO 被移除的ID要通知刷新
                    controller.RemoveFromSlotCallback(closestOverlappingSlot, TileID);
                ...
                {
                    controller.RemoveFromSlotCallback(parent, TileID);
                }
